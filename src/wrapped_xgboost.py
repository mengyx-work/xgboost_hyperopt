...
class xgboost_classifier(object):
    ...
    def fit(self, train = None, label_name = None, use_weights = False, params = None, val = None):
        ...
        print 'the xgboost fit is finished by using {} minutes, saved into {}'.format(round((time.time() - start_time)/60, 1), self.model_file_name)
    ...
    def predict(self, test=None, dep_var_name=None):

        if test is None:
            raise ValueError('test data is not defined.')
	
	if dep_var_name is not None:
	  self.label_name = dep_var_name
	    ...
	    print 'warning! in the prediction step, dep_var_name is not provided....'
